...
import { WhiteTagItem } from '../../types';
import {
  downloadWhiteTagTemplate,
  parseWhiteTagExcel,
  getFormattedTodayDate,
} from '../../utils/module2ExcelService';
import { DeleteConfirmationModal } from './DeleteConfirmationModal';

interface WhiteTagImportTableProps {
  items: WhiteTagItem[];
  setItems?: React.Dispatch<React.SetStateAction<WhiteTagItem[]>>;
  onUpdateItems?: (items: WhiteTagItem[] | ((prev: WhiteTagItem[]) => WhiteTagItem[])) => void;
  onLoadSampleData?: () => void;
  onClearData?: () => void;
  onSwitchToYellow?: () => void;
}

type WhiteSortCol = 'upc' | 'description' | 'price' | 'sku' | 'date' | 'copies';

export const WhiteTagImportTable: React.FC<WhiteTagImportTableProps> = ({
  items,
  setItems,
  onUpdateItems,
  onLoadSampleData,
  onClearData,
  onSwitchToYellow: _onSwitchToYellow,
}) => {
  ...
  const [editForm, setEditForm] = useState<Partial<WhiteTagItem>>({});

  // Table Column Sort State - Defaults to DESCRIPTION A -> Z
  const [sortCol, setSortCol] = useState<WhiteSortCol | null>('description');
  ...
  const updateAllItems = (action: WhiteTagItem[] | ((prev: WhiteTagItem[]) => WhiteTagItem[])) => {
    if (onUpdateItems) {
  ...
  const handleFileUpload = async (file: File) => {
    ...
    try {
      const result = await parseWhiteTagExcel(file);
      if (!result.success) {
        setErrorMessage(result.error || 'Failed to parse White Tag Excel file.');
        return;
      }
  ...
  // Row update helper (for inline copies changes)
  const updateItem = (id: string, field: keyof WhiteTagItem, val: any) => {
    updateAllItems(prev =>
      prev.map(i => {
        if (i.id !== id) return i;
        return { ...i, [field]: val };
  ...
  // Edit Mode Handlers
  const startEdit = (item: WhiteTagItem) => {
    ...
    updateAllItems(prev =>
      prev.map(item => {
        ...
        return {
          ...
          sku: String(editForm.sku ?? item.sku).trim(),
          date: String(editForm.date ?? item.date).trim() || getFormattedTodayDate(),
          price: validPrice,
          copies: validCopies,
        };
      })
  ...
  const requestDeleteItem = (id: string, description?: string) => {
    setDeleteModal({
      ...
      type: 'single',
      id,
      itemName: description || 'White Tag item',
    });
  };
  ...
  // Duplicate row
  const duplicateItem = (item: WhiteTagItem) => {
    const duplicated: WhiteTagItem = {
      ...item,
  ...
  // Add new item
  const addNewItem = () => {
    const newId = `wt-${Date.now()}-${Math.random().toString(36).substring(2, 7)}`;
    const newItem: WhiteTagItem = {
      id: newId,
      upc: '4800000000000',
  ...
  const handleSortClick = (col: WhiteSortCol) => {
    if (sortCol === col) {
      setSortAsc(!sortAsc);
    } else {
      setSortCol(col);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-zinc-50 border-2 border-zinc-200 rounded-2xl p-4 md:p-5 shadow-xs">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <div className="flex items-center gap-2">
              <span className="w-3 h-3 rounded-full bg-zinc-400 ring-4 ring-zinc-200" />
              <h2 className="text-base font-extrabold text-zinc-900">
                White Tag Data & Excel Import
              </h2>
            </div>
            <p className="text-xs text-zinc-600 mt-1 max-w-2xl">
              Upload your Standard White Shelf Tag Excel file containing <span className="font-bold">UPC, DESCRIPTION, PRICE, SKU, DATE, COPIES</span>. Automatically generates Code 128 scannable barcodes and Prince Retail branding.
            </p>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-2">
            <button
              type="button"
              onClick={downloadWhiteTagTemplate}
              className="inline-flex items-center gap-1.5 px-3.5 py-2 rounded-xl text-xs font-bold bg-white text-zinc-800 border border-zinc-300 hover:bg-zinc-100 shadow-2xs transition-all cursor-pointer"
            >
              <Download className="w-4 h-4 text-zinc-600" />
              <span>Download White Template</span>
            </button>

            <button
              type="button"
              onClick={() => fileInputRef.current?.click()}
              className="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl text-xs font-bold bg-zinc-900 hover:bg-zinc-800 text-white shadow-xs transition-all cursor-pointer"
            >
              <Upload className="w-4 h-4" />
              <span>Import White Tag Excel</span>
            </button>
        ...
        <div
          ...
          className={`mt-4 border-2 border-dashed rounded-xl p-4 text-center cursor-pointer transition-all ${
            isDragging
              ? 'border-zinc-500 bg-zinc-100 scale-[0.99]'
              : 'border-zinc-300 hover:border-zinc-400 bg-white'
          }`}
        >
          <div className="flex flex-col items-center justify-center gap-1 text-xs text-zinc-700">
            <FileSpreadsheet className="w-6 h-6 text-zinc-500 mb-1" />
            <span className="font-bold">
              {isProcessing ? 'Processing Excel file...' : 'Drop DEC-White-Tag-Import-Template.xlsx here, or click to browse'}
            </span>
            <span className="text-[11px] text-zinc-500">
              Preserves leading zeroes on UPCs and SKUs • Format: UPC, DESCRIPTION, PRICE, SKU, DATE, COPIES
            </span>
          </div>
        </div>
      ...
      <div className="bg-white border border-zinc-200 rounded-xl p-3 flex flex-wrap items-center justify-between gap-3 text-xs shadow-2xs">
        <div className="flex items-center gap-2 flex-1 min-w-[240px]">
          <div className="relative flex-1 max-w-sm">
            ...
            <input
              type="text"
              placeholder="Search description, UPC, or SKU..."
              value={searchTerm}
              onChange={e => setSearchTerm(e.target.value)}
              className="w-full pl-9 pr-3 py-1.5 border border-zinc-200 rounded-lg text-xs focus:outline-none focus:ring-2 focus:ring-zinc-400"
            />
          </div>
          <button
            type="button"
        ...
        <div className="flex items-center gap-3">
          <div className="flex flex-wrap items-center gap-3 text-zinc-600 text-xs">
            ...
            <span>•</span>
            <span>
              Selected: <strong className="text-zinc-900">{selectedCount}</strong>
            </span>
            <span>•</span>
            <span>
              Physical Tags:{' '}
              ...
              <tr className="bg-zinc-50/80 border-b border-zinc-200 text-zinc-700 font-bold select-none">
                ...
                >
                  <div className="flex items-center gap-1">
                    <span>UPC</span>
                    {sortCol === 'upc' ? (
                      sortAsc ? <ArrowUp className="w-3 h-3 text-zinc-900" /> : <ArrowDown className="w-3 h-3 text-zinc-900" />
                    ) : (
                ...
                <th
                  ...
                  className="py-2.5 px-3 w-28 cursor-pointer hover:bg-zinc-100 transition-colors"
                >
                  <div className="flex items-center gap-1">
                    <span>SKU</span>
                    {sortCol === 'sku' ? (
              ...
              {displayedItems.length === 0 ? (
                <tr>
                  <td colSpan={9} className="py-8 text-center text-zinc-400">
                    No White Tag items found. Import an Excel template or click "Sample Data" above.
                  </td>
                </tr>
              ) : (
                  ...
                  return (
                    <tr
                      ...
                      className={`transition-colors ${
                        isEditing
                          ? 'bg-zinc-100/80 ring-1 ring-zinc-400'
                      ...
                      <td className="py-2 px-3 font-mono">
                        {isEditing ? (
                          <input
                            ...
                            className="w-full font-mono text-zinc-900 px-2 py-1 bg-white border border-zinc-400 rounded focus:ring-1 focus:ring-zinc-500 focus:outline-none"
                      ...
                      <td className="py-2 px-3">
                        {isEditing ? (
                          <input
                            ...
                            className="w-full text-zinc-900 font-semibold px-2 py-1 bg-white border border-zinc-400 rounded focus:ring-1 focus:ring-zinc-500 focus:outline-none"
                      ...
                      <td className="py-2 px-3">
                        {isEditing ? (
                          <input
                            ...
                            className="w-full text-zinc-900 font-mono text-xs px-2 py-1 bg-white border border-zinc-400 rounded focus:outline-none"
                            placeholder="SKU"
                          />
                        ) : (
                          <span className="font-mono text-zinc-700 text-xs font-semibold">
                            {item.sku || '—'}
                          </span>
                        )}
                      </td>
                      ...
                      <td className="py-2 px-3 text-center">
                        {isEditing ? (
                          <input
                            ...
                            className="w-24 text-center font-mono text-xs text-zinc-900 px-1 py-1 bg-white border border-zinc-400 rounded focus:outline-none"
                        ...
                        {isEditing ? (
                          <div className="relative inline-flex items-center w-28">
                            ...
                            <input
                              ...
                              className="w-full text-right font-mono font-bold text-zinc-900 pl-5 pr-2 py-1 bg-white border border-zinc-400 rounded focus:outline-none"
                            />
                          </div>
                        ) : (
                        ...
                        {isEditing ? (
                          ...
                          <div className="inline-flex items-center gap-1">
                            <button
                              ...
                              title="Edit Row"
                              className="p-1 text-zinc-400 hover:text-zinc-900 hover:bg-zinc-100 rounded cursor-pointer transition-colors"
                            >
      ...
      <DeleteConfirmationModal
        ...
        title={
          deleteModal.type === 'single'
            ? 'Delete this White Tag item?'
            : deleteModal.type === 'selected'
            ? `Delete ${deleteModal.count} selected White Tag item(s)?`
            : 'Clear all White Tag items?'
        }
        itemName={deleteModal.type === 'single' ? deleteModal.itemName : undefined}
        message={
          deleteModal.type === 'single'
            ? 'Are you sure you want to delete this White Tag? This item will be removed immediately from the table, Live Preview, and Print/PDF output.'
            : deleteModal.type === 'selected'
